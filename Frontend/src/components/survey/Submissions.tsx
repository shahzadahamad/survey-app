    ...
    const getNationalities = async () => {
      try {
        if (role === UserRoles.ADMIN) {
          const response: SubmissionType[] = await listAllSubmissions();
          setSubmissions(response);
        } else if (role == UserRoles.USER) {
          const response: SubmissionType[] = await listUserSubmissions();
          setSubmissions(response);
        }
      } catch (error) {
      ...
      <div className="w-full py-6 px-2 sm:px-8 transition-all duration-500 transform translate-y-0 opacity-100">
        {role === 'user' && (
          <div className="flex justify-end mb-4">
            <button onClick={() => goToHome()} className="bg-gray-500 hover:bg-gray-600 text-white py-2 px-4 rounded-md transition-all text-sm">Back</button>
          </div>
        )}
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-4 gap-2">
          ...
          <div className="relative w-full sm:w-auto">
            <input type="text" placeholder="Search submissions..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="pl-10 pr-4 py-2 border border-gray-300 rounded-md w-full sm:w-64 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500" />
          ...
          <div className="flex items-center">
            ...
            <select value={itemsPerPage} onChange={(e) => setItemsPerPage(Number(e.target.value))} className="border border-gray-300 rounded-md py-1 px-2 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 text-sm">
            ...
            <thead className="bg-gray-100">
              <tr>
                {columnHeaders.map((column) => (
                  <th key={column.key} className="py-3 px-4 text-left text-xs font-medium text-gray-500 uppercase tracking-wider cursor-pointer hover:bg-gray-200 transition-colors" onClick={() => requestSort(column.key as keyof SubmissionType)}>
                    <div className="flex items-center">
                      {column.label}
        ...
        <div className="flex flex-col sm:flex-row justify-between items-center gap-2">
          ...
          <div className="flex flex-wrap justify-center gap-1">
            <button onClick={() => paginate(1)} disabled={currentPage === 1} className={`py-1 px-3 border rounded-md text-sm ${currentPage === 1 ? 'bg-gray-100 text-gray-400 cursor-not-allowed' : 'bg-white text-gray-700 hover:bg-gray-50'}`}>  First</button>
            <button onClick={() => paginate(currentPage - 1)} disabled={currentPage === 1} className={`py-1 px-3 border rounded-md text-sm ${currentPage === 1 ? 'bg-gray-100 text-gray-400 cursor-not-allowed' : 'bg-white text-gray-700 hover:bg-gray-50'}`}>  Previous </button>
            {renderPaginationButtons()}
            <button onClick={() => paginate(currentPage + 1)} disabled={currentPage === totalPages || totalPages === 0} className={`py-1 px-3 border rounded-md text-sm ${currentPage === totalPages || totalPages === 0 ? 'bg-gray-100 text-gray-400 cursor-not-allowed' : 'bg-white text-gray-700 hover:bg-gray-50'}`}>  Next</button>
            <button onClick={() => paginate(totalPages)} disabled={currentPage === totalPages || totalPages === 0} className={`py-1 px-3 border rounded-md text-sm ${currentPage === totalPages || totalPages === 0 ? 'bg-gray-100 text-gray-400 cursor-not-allowed' : 'bg-white text-gray-700 hover:bg-gray-50'}`}> Last</button>
          </div>
        </div>
      </div>
    </div>
